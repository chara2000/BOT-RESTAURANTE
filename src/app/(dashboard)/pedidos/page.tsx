...
export default function PedidosPage() {
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full overflow-hidden">
      ...
      {/* Controls / Filter Hub */}
      <div className="px-5 lg:px-8 pt-4 z-10 relative">
        <div 
          className="card p-3 sm:p-4 rounded-3xl border shadow-sm backdrop-blur-md bg-[var(--bg-card)]/90 flex flex-col xl:flex-row xl:items-center justify-between gap-4 transition-all duration-200"
          style={{ borderColor: 'var(--border)' }}
        >
          {/* Left Side: View Mode & Shift Scope */}
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Scope Toggle: Turno Caja vs Todo el Día */}
            <div className="flex items-center bg-[var(--bg-input)] rounded-2xl p-1 border shadow-inner" style={{ borderColor: 'var(--border)' }}>
              <button
                type="button"
              ...
              >
                ...
                <span>Turno Caja</span>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${
                  dayScope === 'shift' ? 'bg-white/20 text-white' : 'bg-[var(--bg-card)] text-[var(--text-muted)] border border-[var(--border)]'
                }`}>
                  {shiftOrdersCount}
                </span>
              </button>
              ...
              >
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${
                  dayScope === 'today' ? 'bg-white/20 text-white' : 'bg-[var(--bg-card)] text-[var(--text-muted)] border border-[var(--border)]'
                }`}>
                  {todayOrdersCount}
                </span>
              </button>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="relative flex-1 sm:flex-initial sm:w-60">
              ...
              <input 
                ...
                className="w-full pl-9 pr-8 py-2 rounded-2xl text-xs font-bold bg-[var(--bg-input)] border text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--orange)] focus:ring-1 focus:ring-[var(--orange)] outline-none transition-all"
                style={{ borderColor: 'var(--border)' }}
              />
              {filterSearch && (
                <button
                  ...
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded-full text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] transition-colors"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
            ...
            {/* Status Filter (when in list mode) */}
            {viewMode === 'list' && (
              <div className="relative">
                <select 
                  value={filterStatus} 
                  onChange={(e) => setFilterStatus(e.target.value)}
                  className="pl-3.5 pr-8 py-2 rounded-2xl text-xs font-bold bg-[var(--bg-input)] border text-[var(--text-primary)] focus:border-[var(--orange)] focus:ring-1 focus:ring-[var(--orange)] outline-none transition-all cursor-pointer appearance-none"
                  style={{ borderColor: 'var(--border)' }}
                >
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="dine_in">🍽️ Mesa</option>
            ...
            <div className="relative">
              <select 
                ...
                className="pl-3.5 pr-8 py-2 rounded-2xl text-xs font-bold bg-[var(--bg-input)] border text-[var(--text-primary)] focus:border-[var(--orange)] focus:ring-1 focus:ring-[var(--orange)] outline-none transition-all cursor-pointer appearance-none max-w-[170px] truncate"
                style={{ borderColor: 'var(--border)' }}
              >
            ...
            {(filterSearch || filterType !== 'all' || filterCategory !== 'all' || (viewMode === 'list' && filterStatus !== 'all')) && (
              <button
                type="button"
                onClick={() => {
                  setFilterSearch('');
                ...
                className="flex items-center gap-1 px-3 py-2 rounded-2xl text-xs font-bold text-rose-500 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/25 transition-all cursor-pointer"
              >
            ...
            {/* Botón Crear Pedido integrado en el Hub para desktop */}
            <button
              type="button"
              onClick={() => setShowCreateModal(true)}
              className="hidden sm:flex items-center gap-2 text-xs font-black px-4 py-2 rounded-2xl text-white shadow-sm hover:opacity-95 active:scale-95 transition-all cursor-pointer ml-auto xl:ml-0"
