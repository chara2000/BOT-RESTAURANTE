...
import { Edit2, Plus, Search, Trash2, ToggleLeft, ToggleRight, LayoutGrid } from 'lucide-react';
import { Topbar } from '@/components/layout/Topbar';
import { useAppData } from '@/context/AppDataContext';
import { formatCurrency } from '@/lib/utils';
import type { Product } from '@/types';

const CATEGORIES = ['Todos', 'Hamburguesas', 'Pizzas', 'Entradas', 'Mariscos', 'Bebidas', 'Postres', 'Acompañamientos'];
...
export default function MenuPage() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setEditing(null);
      setShowForm(false);
      setMessage(editing ? 'Producto actualizado y guardado en Supabase.' : 'Producto creado y guardado en Supabase.');
    } catch (err) {
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 lg:p-8 space-y-6 lg:space-y-8 z-10 relative">
        <div className="flex flex-col gap-5 animate-fade-in-up">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button onClick={() => { setEditing(null); setShowForm(true); }}
              className="flex items-center justify-center gap-2 text-sm font-black px-6 py-3.5 rounded-xl text-white shadow-[0_8px_20px_var(--orange-glow)] hover:scale-105 active:scale-95 transition-all shrink-0"
        ...
        {message && (
          <div className={`p-4 rounded-2xl border backdrop-blur-md font-bold text-sm animate-fade-in-up ${message.includes('guardado') ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-rose-500/10 text-rose-500 border-rose-500/20'}`}>
            {message}
          </div>
        )}
        ...
        {showForm && (
          <form onSubmit={handleSave} className="card p-6 grid grid-cols-1 md:grid-cols-2 gap-5 animate-fade-in-up">
            ...
            <select name="category" defaultValue={editing?.category ?? 'Hamburguesas'}
              className="text-sm font-semibold px-4 py-3 rounded-xl border focus:outline-none focus:ring-2 focus:ring-[var(--orange)]" style={{ background: 'var(--bg-input)', borderColor: 'var(--border)' }}>
            ...
            <div key={product.id} className="card overflow-hidden group hover:shadow-lg transition-all duration-300 flex flex-col">
              <div className="relative h-44 overflow-hidden">
                ...
                <button onClick={async () => {
                  setMessage(null);
                  try {
                    await updateProduct({ ...product, is_available: !product.is_available });
                  } catch (err) {
                    setMessage(err instanceof Error ? err.message : 'Error al actualizar disponibilidad');
                  }
                }}
